...
function estimateMessagesTokens(messages: ChatMessage[]): number {
  ...
  for (const m of messages) {
    if (typeof m.content === "string") {
      ...
      // Multimodal content - estimate text and images
      for (const item of m.content) {
        if (item.type === "text" && item.text) {
          totalChars += item.text.length;
        } else if (item.type === "image_url") {
          // Images typically use ~85 tokens per 512x512 tile (OpenAI pricing)
          // Estimate conservatively at 200 tokens per image
          totalChars += 800; // ~200 tokens * 4 chars/token
        }
      }
    }
    if (m.tool_calls) {
...
function checkAndTruncateContext(
  ...
  if (percentUsed > 70) {
    console.log(
      `[Mantle:Context] ⚠️ Context usage: ${Math.round(percentUsed)}% (${totalTokens}/${contextWindow} tokens)`,
    );
  }
  ...
  console.log(
    `[Mantle:Context] 🔧 Compacting context: ${totalTokens} > ${budget} tokens`,
  );
  ...
  console.log(
    `[Mantle:Context] Removed ${removedCount} old messages, new usage: ${newTokens} tokens`,
  );
...
export async function* streamBedrockMantle(
  ...
  const toolDefinitions =
    tools.length > 0 ? tools.map(convertToolToOpenAIFormat) : undefined;
  ...
  const initialContext = checkAndTruncateContext(
    conversationMessages,
    contextWindow,
  );
  conversationMessages = initialContext.messages;
  console.log(
    `[BedrockMantle] Initial context: ${initialContext.estimatedTokens} tokens (${Math.round(initialContext.percentUsed)}%)`,
  );
  ...
  try {
    while (iteration < maxIterations) {
      ...
      const contextCheck = checkAndTruncateContext(
        conversationMessages,
        contextWindow,
      );
      ...
      if (!response.ok) {
        ...
        console.error(
          `[BedrockMantle] API error: ${response.status} ${response.statusText}`,
        );
      ...
      if (finishReason === "tool_calls" && toolCalls.length > 0) {
        console.log(
          `[BedrockMantle] Model requested ${toolCalls.length} tool calls`,
        );
...
export async function completeBedrockMantle(
  ...
  if (!response.ok) {
    throw new Error(
      `Bedrock Mantle API error: ${response.status} ${response.statusText}`,
    );
  }
