...
export class SkillService {
   ...
   */
  async list(workspaceId: string): Promise<(typeof schema.workspaceSkills.$inferSelect)[]> {
    return this.db
      .select()
  ...
  private parseSkillFile(content: string): ParsedSkillFile {
    ...
    return {
      frontmatter,
      content,
      isValid: errors.length === 0 && !!frontmatter.name && !!frontmatter.description,
      errors,
    };
  }

  private validateSkillName(name: string): Array<{ field: string; message: string }> {
    const errors: Array<{ field: string; message: string }> = [];
