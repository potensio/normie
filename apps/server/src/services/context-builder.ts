 ...
 * Loads SOUL.md, MEMORY.md, recent memories, and daily note
 * Note: AGENTS.md is intentionally excluded (internal developer docs only)
 *
...
export async function buildSystemContext(
  ...
  // NOTE: AGENTS.md is intentionally NOT included in AI context
  // It contains internal developer documentation and should not be exposed to AI
...
export async function buildFullContext(
  ...
  const { maxMessages = 20, pendingAttachments } = options;
  ...
  if (chatId) {
    try {
      ...
      // Note: Attachment paths are already in message content, no need to load from DB
  ...
  if (pendingAttachments && pendingAttachments.length > 0) {
    // Inject file paths directly (like in terminal - no wrapper text)
    const attachmentPaths = pendingAttachments.map((a) => a.path).join("\n");
